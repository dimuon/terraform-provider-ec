...
func Test_writeRemoteClusters(t *testing.T) {
	type args struct {
		remoteClusters ElasticsearchRemoteClusters
	}
	tests := []struct {
		name string
		args args
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			diags := tfsdk.ValueFrom(context.Background(), tt.args.remoteClusters, elasticsearchRemoteCluster().Type(), &remoteClustersTF)
			assert.Nil(t, diags)
